...
const cats=new Map(); const idsByCategory=new Map();
for(const p of PRODUCT_INDEX){
 const id=String(p.productId||p.id||''); const c=String(p.category||'unknown');
 if(!id) failures.push('missing productId');
 else {
   if(!idsByCategory.has(c)) idsByCategory.set(c,new Set());
   const ids=idsByCategory.get(c);
   if(ids.has(id)) failures.push(`duplicate productId within category ${c}: ${id}`); else ids.add(id);
 }
