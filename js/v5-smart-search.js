...
function init(){
 ...
 hint.textContent=hints[0]; timer=setInterval(()=>{i=(i+1)%hints.length;hint.classList.remove('v5-hint-show');void hint.offsetWidth;hint.textContent=hints[i];hint.classList.add('v5-hint-show');},2600);
 input.addEventListener('focus',()=>hint.style.opacity='0'); input.addEventListener('blur',()=>{if(!input.value)hint.style.opacity='1'});
 form.addEventListener('submit',async e=>{e.preventDefault();const q=input.value.trim();if(!q)return;hint.style.opacity='0';const old=input.placeholder;input.placeholder='در حال جستجوی زنده...';input.disabled=true;
  ...
  try{const r=await fetch(API_URL+'?q='+encodeURIComponent(q));const d=await r.json();if(!d.success||!d.results?.length){box.innerHTML='<div class="v5-smart-search-empty">برای این جستجو نتیجه‌ای پیدا نشد. عبارت دیگه‌ای رو امتحان کن.</div>';return;}
   box.innerHTML='<div class="v5-smart-search-result-head">نتایج زنده برای «'+esc(q)+'»</div>'+d.results.map(p=>'<article class="v5-smart-result"><div class="v5-smart-result-title">'+esc(p.title)+'</div><div class="v5-smart-result-meta">'+esc(p.storeName||'فروشگاه')+' · '+price(p.price)+'</div><a target="_blank" rel="noopener noreferrer" href="'+esc(affiliate(p))+'">مشاهده و خرید</a></article>').join('');
