/* =========================================================
   ...
   Build 15 — canonical toman pricing
   ========================================================= */
...
(function () {
  ...
  function resolveImage(product) {
    ...
    if (Array.isArray(images)) values.push.apply(values, images);
    else if (images && typeof images === "object") values.push(images.main, images.primary, images.thumbnail, images.large, images.medium, images.small);
    return firstNonEmpty(values);
  }

  function resolveProductUrl(product) {
    if (!product || typeof product !== "object") return "";
    return absoluteDigikalaUrl(firstNonEmpty([product.productUrl, product.product_url, product.url, product.web_url, product.link]));
  }
  ...
  /* DigiKala's monetary API values are in rial. DigiYar's canonical unit is toman. */
  function normalizePrice(value, currency) {
    const number = safeNumber(value);
    if (number <= 0) return 0;
    if (currency === "rial") return Math.round(number / 10);
  ...
  function resolveName(product) {
    return firstNonEmpty([product && product.title_fa, product && product.name, product && product.title, product && product.product_name, product && product.title_en]);
  }
  ...
  function normalizeProduct(product, options) {
    ...
    const id = product.id || product.pk || product.product_id || product.code || "";
  ...
  function localSearch(query, options) {
    ...
    return results.map(function (product) { return normalizeProduct(product, { currency: "toman", store: "local" }); })
  ...
  function extractDigikalaProducts(data) {
    if (!data) return [];
    const candidates = [data.data && data.data.products, data.data && data.data.items, data.products, data.items, data.data && data.data.products && data.data.products.data, data.data && data.data.products && data.data.products.items];
    let products = [];
    for (let i = 0; i < candidates.length; i++) {
      if (Array.isArray(candidates[i])) { products = candidates[i]; break; }
    }
    return products.map(function (product) { return normalizeProduct(product, { currency: CONFIG.digikalaRemoteCurrency, store: "digikala" }); })
      .filter(function (product) { return product && product.name; })
      .slice(0, CONFIG.maxResults);
  }
  ...
  async function searchRemote(query) {
    ...
    const directUrl = CONFIG.digikalaSearchEndpoint + encodeURIComponent(normalizedQuery);
  ...
  async function search(query, options) {
    ...
    try {
      ...
      console.warn("DigiYar Product Retrieval: live source unavailable; using local fallback.", error);
    }
