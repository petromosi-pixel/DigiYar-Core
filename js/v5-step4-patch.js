...
function init(){
 ...
 /* Budget belongs immediately below the product selector (subcategory). */
 ...
 function syncIcon(open){
   const icon=t.querySelector('.v5-step4-toggle-icon');
   if(icon)icon.textContent=open?'⌃':'⌄';
   t.setAttribute('aria-expanded',String(open));
   t.setAttribute('aria-label',open?'بستن جزئیات خرید':'باز کردن جزئیات خرید');
 }
 ...
 function clearResults(){
   ['digiyar-products','recommendations'].forEach(id=>{const el=document.getElementById(id);if(el)el.innerHTML='';});
   const inline=document.getElementById('v5InlineResults');if(inline)inline.hidden=true;
   const summary=document.getElementById('needSummary');if(summary){summary.innerHTML='';summary.classList.add('empty');summary.hidden=true;}
   const result=document.getElementById('resultSection');if(result)result.hidden=true;
 }
 function clearInputsOnly(){
   store.value='';
   cat.value='';cat.disabled=true;
   if(sub){sub.value='';sub.disabled=true;sub.innerHTML='<option value="">انتخاب کالا</option>';}
   budget.value='';
   dyn.innerHTML='';dyn.hidden=true;dyn.style.display='none';
   if(catField)catField.hidden=true;
   if(subField)subField.hidden=true;
   budgetState();
   clearResults();
   /* Intentionally do NOT call setOpen(): clearing data never changes card state. */
 }

 /* Remove any older reset handler and make Clear data-only, including results. */
 if(reset){
   const freshReset=reset.cloneNode(true);reset.replaceWith(freshReset);
   freshReset.addEventListener('click',function(ev){ev.preventDefault();ev.stopImmediatePropagation();clearInputsOnly();},true);
 }
 /* The details button is the ONLY open/close controller. */
 ...
 /* Keep every function's label inside its select as the placeholder. */
 function normalizeFunctionLabels(){
   dyn.querySelectorAll('.v5-final-function,.v5-step4-field').forEach(field=>{
     const title=field.querySelector(':scope > span');
     const select=field.querySelector('select');
     if(!select)return;
     if(title){const text=title.textContent.trim();if(text){const first=select.options[0];if(first)first.textContent=text;else{const o=document.createElement('option');o.value='';o.textContent=text;select.prepend(o);}title.remove();}}
     select.style.textAlign='center';
     Array.from(select.options).forEach(o=>o.style.textAlign='center');
   });
 }
 ...
 /* Let the original app submit handler run, but make sure the results section is visible after submission. */
 form.addEventListener('submit',function(){
   setOpen(true);
   window.setTimeout(function(){
     const result=document.getElementById('resultSection');
     const inline=document.getElementById('v5InlineResults');
     if(result)result.hidden=false;
     if(inline)inline.hidden=false;
   },50);
 },false);

 /* Initial state: closed, but the selected store control remains visible. */
 setOpen(false);
}
