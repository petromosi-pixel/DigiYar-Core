...
function init(){
 ...
 if(sub){const label=subField&&subField.querySelector('span');if(label)label.remove();}
 if(budgetField){const label=budgetField.querySelector('span');if(label)label.textContent='چقدر می‌خوای هزینه کنی؟';}
 card.appendChild(t);
 function moveLabelInside(select,labelText){if(!select)return;let first=select.options[0];if(!first){first=document.createElement('option');first.value='';select.prepend(first);}first.textContent=labelText;select.style.textAlign='center';select.style.textAlignLast='center';Array.from(select.options).forEach(o=>o.style.textAlign='center');}
