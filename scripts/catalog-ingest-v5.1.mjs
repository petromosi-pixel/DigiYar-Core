...
function anchors(html,base,kind){const out=[];const re=/<a\b[^>]*href=["']([^"']+)["'][^>]*>([\s\S]*?)<\/a>/gi;let m;while((m=re.exec(html))){const url=abs(base,m[1]);const text=norm(m[2].replace(/<[^>]+>/g,' '));if(!url||!text)continue;if(kind==='shop' && /\/product(?:-|\/)|\/p\//i.test(url))out.push({url,name:text,near:html.slice(m.index,Math.min(html.length,m.index+1800))});if(kind==='market' && /mobile\.ir\/phones\//i.test(url))out.push({url,name:text,near:html.slice(m.index,Math.min(html.length,m.index+1600))})}return out}
function parseHtml(html,source,category){const found=[];for(const x of jsonLd(html))found.push({name:x.name,url:x.url,sku:x.sku,price:x.price,availability:x.availability,source});for(const x of anchors(html,source.url,source.kind)){const near=norm(x.near);const p=money((near.match(/(?:تومان|IRR)[^\d]{0,50}([\d٬,]{5,})/i)||near.match(/([\d٬,]{5,})\s*(?:تومان|IRR)/i)||[])[1]);found.push({name:x.name,url:x.url,price:p,availability:availability(near),source})}return found.map(x=>{const url=abs(source.url,x.url);const id=String(x.sku||url||x.name||'');return{id:`web-${category}-${hash(id)}`,productId:`web-${category}-${hash(id)}`,name:norm(x.name),brand:brand(x.name),model:norm(x.name),category:categoryName(category),price:Number(x.price)||0,currency:x.currency||'IRR',availability:String(x.availability||'').toLowerCase().includes('outofstock')?'out_of_stock':availability(`${x.availability} ${x.name}`),productUrl:url,sourceId:source.id,sourceUrl:source.url,source:`web-catalog:${source.id}`,observedAt:new Date().toISOString()}}).filter(x=>x.name&&x.productUrl)}

async function collect(category){const all=[];for(const s of SOURCES[category]){const pages=s.pages||1;for(let page=1;page<=pages;page++){let url=s.url;if(page>1){url=url.endsWith('/')?`${url}page/${page}/`:`${url}/page/${page}/`}try{const html=await get(url);all.push(...parseHtml(html,{...s,url},category))}catch(e){console.warn(`skip ${url}: ${e.message}`)}}}const uniq=[];const seen=new Set();for(const x of all){const key=x.productUrl||x.name;if(seen.has(key))continue;seen.add(key);uniq.push(x);if(uniq.length>=MAX)break}return{version:'5.1',category,maxProducts:MAX,generatedAt:new Date().toISOString(),sources:SOURCES[category],products:uniq,status:'web-ingested',count:uniq.length}}
