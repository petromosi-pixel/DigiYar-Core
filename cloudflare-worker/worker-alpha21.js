  ...
  async fetch(request) {
    ...
    if (url.pathname === "/health") {
      return json({
        ...
        version: VERSION,
        upstream: "digikala",
        strategy: "search_page_product_id_discovery_v2"
      });
    }
    ...
    if (url.pathname === "/autocomplete") {
      return json({
        ...
        query,
        error: "Autocomplete disabled in alpha.21 discovery build",
        diagnostics: { strategy: "search_page_product_id_discovery_v2" }
      }, 501);
    }
...
function browserHeaders() {
  return {
    ...
    "User-Agent": "Mozilla/5.0 (Linux; Android 16) AppleWebKit/537.36 Chrome/143 Mobile Safari/537.36",
    "Referer": DIGI_BASE + "/",
    "Origin": DIGI_BASE
  };
}
  ...
  try {
    ...
    return json({
      ...
      diagnostics: {
        ...
        productIdCount: ids.length,
        strategy: "fetch_search_page_extract_dkp_ids"
      },
      productIds: ids
    }, response.ok && ids.length > 0 ? 200 : 502);
  } catch (error) {
    return json({
      ok: false,
      endpoint: "/discovery",
      ...
      diagnostics: { stage: "search_page", pageUrl, strategy: "fetch_search_page_extract_dkp_ids" }
    }, 502);
  }
}
  ...
  try {
    ...
    if (!response.ok || ids.length === 0) {
      return json({
        ...
        query,
        source: "digikala",
        error: response.ok ? "No product IDs found in search page" : "Search page unavailable",
        diagnostics: {
          stage: "search_page",
          pageUrl,
          finalUrl,
    ...
    return json({
      ...
      diagnostics: {
        ...
        pageUrl,
        searchPageStatus: response.status,
        searchPageHtmlBytes: html.length,
        discoveredIds: ids.length,
        detailLimit: DETAIL_LIMIT,
        completedDetails: products.length,
        strategy: "search_page_product_id_discovery_v2",
...
function extractProductIds(html) {
  ...
  const seen = new Set();
  const patterns = [
    /(?:https?:\\/\\/www\\.digikala\\.com)?\\/product\\/dkp-(\\d+)/gi,
    /dkp-(\\d+)/gi
  ];
...
async function getProductDetail(id) {
  const response = await fetch(`${API_BASE}/v2/product/${id}/`, {
    method: "GET",
    redirect: "follow",
    headers: apiHeaders()
  });
...
function normalizeProduct(payload, fallbackId) {
  ...
  return {
    ...
    title: p.title_fa || p.title_en || "",
    price: price.selling_price ?? variant.price?.selling_price ?? null,
    rrpPrice: price.rrp_price ?? variant.price?.rrp_price ?? null,
    discountPercent: price.discount_percent ?? variant.price?.discount_percent ?? null,
    currency: "IRR",
    url: p.url?.uri ? DIGI_BASE + p.url.uri : `${DIGI_BASE}/product/dkp-${id}/`,
    image: mainImage,
    brand: p.data_layer?.brand || null,
    category: p.data_layer?.item_category3 || p.data_layer?.item_category2 || null,
    rating: p.rating?.rate ?? null,
    ratingCount: p.rating?.count ?? null,
    status: p.status || variant.status || null,
    stock: price.marketable_stock ?? null
  };
}
