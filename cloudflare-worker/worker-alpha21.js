...
export default { async fetch(request, env) {
  ...
  if (url.pathname === "/health") return json({ok:true,service:"DigiYar Core",version:VERSION,upstream:"digikala",strategy:"canonical_search_api_then_product_hydration",tracking:"analytics_engine"});
...
async function track(request, env){
  try{
    ...
    env.ANALYTICS.writeDataPoint({blobs:["affiliate_click",productId,String(body?.brand||""),String(body?.model||""),String(body?.variant||""),String(body?.affiliate_network||""),affiliateUrl,String(body?.need_category||""),String(body?.budget||""),eventId],doubles:[1],indexes:[productId]});
...
async function apiSearch(q){const candidates=[`${API_BASE}/v1/search/?q=${encodeURIComponent(q)}&page=1`,`${API_BASE}/v2/search/?q=${encodeURIComponent(q)}&page=1`];const attempts=[];for(const endpoint of candidates){try{const r=await fetch(endpoint,{redirect:"manual",headers:headers()});const location=r.headers.get("location");const text=await r.text();attempts.push({endpoint,status:r.status,location:location||null,contentType:r.headers.get("content-type")||"",bytes:text.length});if(r.status>=300&&r.status<400)continue;if(!r.ok)continue;let data;try{data=JSON.parse(text);}catch(_){continue;}const products=normalizeProducts(data);if(products.length)return {products,attempts,rawShape:shape(data)};}catch(e){attempts.push({endpoint,error:e instanceof Error?e.message:String(e)})}}return {products:[],attempts};}
