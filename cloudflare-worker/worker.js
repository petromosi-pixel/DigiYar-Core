...
function walk(value, out, seen, depth) { if (depth > 18 || value == null || out.length >= 20) return; if (Array.isArray(value)) { for (const x of value) walk(x, out, seen, depth + 1); return; } if (typeof value !== "object") return; const product = normalizeProduct(value); if (product && !seen.has(String(product.id))) { seen.add(String(product.id)); out.push(product); } for (const x of Object.values(value)) walk(x, out, seen, depth + 1); }
function normalizeProduct(item) { if (!item || typeof item !== "object" || Array.isArray(item)) return null; const id = item.id ?? item.productId ?? item.product_id ?? extractId(item.url); const title = item.title_fa || item.product_title_fa || item.name_fa || item.title || item.name || item.productTitle || item.displayName; const price = findMoney(item, 0); if (!id || !title || !price) return null; return { id: Number(id) || id, title: String(title).trim(), price, rrpPrice: findRrp(item, 0) || null, currency: "IRR", url: productUrl(id, item.url || item.productUrl || item.link), image: findImage(item, 0), rating: item.rating?.rate ?? item.rating?.stars ?? item.rating_stars ?? item.rating ?? null, ratingCount: item.rating?.count ?? item.rating_count ?? null, brand: item.brand?.title_fa ?? item.brand?.title ?? item.brand ?? null, status: item.status ?? null }; }
function productUrl(id, value) { const text = String(value || "").trim(); if (/^https?:\/\//i.test(text)) return text; if (text) return DIGI_BASE + (text.startsWith("/") ? text : "/" + text); return `${DIGI_BASE}/product/dkp-${id}/`; }
function findMoney(value, depth) { if (depth > 8 || value == null) return 0; if (typeof value === "number") return value >= 10000 ? value : 0; if (typeof value === "string") { const n = parseMoney(value); return n >= 10000 ? n : 0; } if (typeof value !== "object") return 0; for (const key of ["selling_price", "sellingPrice", "selling_price_rial", "finalPrice", "salePrice", "defaultPrice", "price"]) { if (value[key] != null) { const n = parseMoney(value[key]); if (n >= 10000) return n; } } for (const key of ["default_variant", "defaultVariant", "variants", "variant", "offers", "offer", "prices", "price"]) { if (value[key] != null) { const n = findMoney(value[key], depth + 1); if (n) return n; } } return 0; }
...
async function enrichProducts(products) {
  const results = products.slice(); const candidates = results.slice(0, DETAIL_LIMIT); const concurrency = 5;
  for (let i = 0; i < candidates.length; i += concurrency) {
    const batch = candidates.slice(i, i + concurrency);
    const enriched = await Promise.all(batch.map(async p => { const detail = await getProductDetail(p.id); if (detail) { p.image = findImage(detail, 0) || p.image || null; const detailUrl = extractProductUrl(detail); if (detailUrl) p.url = detailUrl; } p.url = p.url || productUrl(p.id, null); return p; }));
    enriched.forEach(p => { const index = results.findIndex(x => String(x.id) === String(p.id)); if (index >= 0) results[index] = p; });
  }
  return results;
}
async function getProductDetail(id) { for (const path of [`/v2/product/${id}/`, `/v1/product/${id}/`]) { const result = await fetchJson(API_BASE + path); if (result.ok) return result.data; } return null; }
